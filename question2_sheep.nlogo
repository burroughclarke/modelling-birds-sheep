...
to setup-sheep
  ...
  foreach (range 1 (length region-boundaries + 1)) [ region-number ->
    ...
    create-sheep number-of-turtles-per-region [
      move-to one-of region-patches

      set shape  "sheep"
      set color white
      set size 1.5
      set label-color black - 2
...
to eat-grass  ; sheep procedure
  ; sheep eat grass, turn the patch black
  if pcolor = green [
    set pcolor black
...
to grow-grass  ; countdown on black patches: if reach 0, grow some grass
